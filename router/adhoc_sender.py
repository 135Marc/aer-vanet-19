...
def sender(socket, name, port, groupipv6, routing_table, interval, msgqueue, rplyawait, answers):
    ...
    rr = threading.Thread(target=dispatch, args=(sock,msgqueue,groupipv6,port,rplyawait,interval,name, answers,))
    rr.start()
...
def dispatch(sock, msgqueue, groupipv6, port, rplyawait, interval, name, answers):
    while True:
        # Obter proxima mensagem
        pdu = msgqueue.get()
        print('sender ' + pdu.getType())
        print('sender ' + pdu.getTarget())
        print('sender ' + pdu.getMsg())
        ...
        if(pdu.getType() == 'ROUTE_REQUEST'):
            ...
            t = threading.Thread(target=rmAwaitPdu, args=(rplyawait,pdu.getTarget(), interval, answers,))
            t.start()
